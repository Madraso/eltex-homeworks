...
int main() {
    ...
    int serv_fd = creat_sock(&server, SERV_PORT),
        st_size = sizeof(client), cl_cnt = 0;
    ...
    for (int i = 0; i < THREAD_CNT; i++) {
        sem_init(&servers->sem, 0, 0);
        pthread_create(&tid, NULL, hdl_client, (void *) &servers[i]);
    }
    ...
    while (1) {
        ...
        if (cl_cnt > THREAD_CNT) {
            ...
            servers[cl_cnt - 1].fd = cl_fd;
            sem_post(&servers[cl_cnt - 1].sem);
        }
    }
}